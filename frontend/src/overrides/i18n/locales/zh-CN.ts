...
export default {
    ...
    canvas: {
        ...
        apiKeys: {
            ...
            bindingTitle: "创作台 API Key 绑定",
            bindingDescription: "选择允许创作台使用的 Sub2API Key。Key 只由服务端读取并加密保存，不会返回到浏览器。",
            bindingLoadFailed: "无法读取 API Key",
            bind: "绑定",
            bound: "API Key 已绑定",
            boundStatus: "已绑定",
            bindConfirmTitle: "将 {{name}} 绑定到创作台？",
            bindConfirmDescription: "之后的生成请求将使用该 Key 的权限、额度和账单归属。独立创作台会使用 AES-256-GCM 加密保存。",
            unbind: "解除绑定",
            unbound: "已解除 API Key 绑定",
            unbindConfirm: "确定解除这个 API Key 与创作台的绑定？",
            noOfficialKeys: "你的 Sub2API 账户还没有 API Key",
            active: "可用",
            usage: "用量 {{used}} / {{total}}",
            refresh: "刷新",
            back: "返回画布列表",
